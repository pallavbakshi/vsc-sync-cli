...
class SetupProjectCommand:
    ...
    def run(
        ...
        try:
            console.print(f"[bold blue]Setting up .vscode/ for project:[/bold blue] {project_path}")
    ...
    def _merge_project_layers(
        ...
        if from_project_type:
            if self.layer_manager.layer_exists("project", from_project_type):
                project_layer = LayerInfo(
                    ...
                    path=self.layer_manager.get_layer_path("project", from_project_type)
                )
                layers.append(project_layer)
            else:
                raise LayerNotFoundError(f"Project type '{from_project_type}' not found")
        ...
        if not layers:
            raise VscSyncError("No layers specified. Use --from-project-type or --stack to specify configuration sources.")
        ...
        for layer in layers:
            ...
            if settings_file.exists():
                ...
                merged_settings = self.layer_manager.deep_merge_dicts(merged_settings, layer_settings)
    ...
    def _write_project_files(self, vscode_dir: Path, merge_result: MergeResult) -> None:
        ...
        if merge_result.extensions:
            ...
            extensions_config = {
                "recommendations": merge_result.extensions
            }
            FileOperations.write_json_file(extensions_file, extensions_config)
            console.print(f"[green]✓[/green] Created {extensions_file}")
    
    def _show_success_message(self, project_path: Path, merge_result: MergeResult) -> None:
