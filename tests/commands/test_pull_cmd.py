...
class TestPullCommand:
    ...
    def temp_dirs(self):
        ...
        with tempfile.TemporaryDirectory() as app_config_dir, \
             tempfile.TemporaryDirectory() as vscode_configs_dir:
            yield {
    ...
    def test_validate_app_config_path_not_exists(self, temp_dirs):
        ...
        config = VscSyncConfig(
            ...
            managed_apps={
                "test-app": AppDetails(
                    alias="test-app",
                    config_path=temp_dirs["app_config"] / "nonexistent",  # This doesn't exist
                    executable_path=Path("/usr/bin/test-app"),
                )
            },
        )
        ...
        with pytest.raises(AppConfigPathError, match="App config directory does not exist"):
    ...
    def test_resolve_target_layer_path_app_with_custom_name(self, pull_command, temp_dirs):
        """Test resolving app layer path with custom name."""
        target_path = pull_command._resolve_target_layer_path("app", "custom-name", "test-app")
    ...
    def test_resolve_target_layer_path_stack(self, pull_command, temp_dirs):
        ...
        target_path = pull_command._resolve_target_layer_path("stack", "python", "test-app")
    ...
    def test_resolve_target_layer_path_stack_no_name(self, pull_command):
        ...
        with pytest.raises(VscSyncError, match="Layer name is required for stack layer type"):
    ...
    def test_pull_extensions_success(self, mock_get_extensions, pull_command, temp_dirs):
        """Test successful extensions pull."""
        # Mock installed extensions
        mock_get_extensions.return_value = ["ms-python.python", "ms-vscode.vscode-typescript-next"]
    ...
    def test_run_dry_run_vs_actual(self, mock_console, pull_command, temp_dirs, dry_run):
    ...
    def test_pull_keybindings_success(self, pull_command, temp_dirs, app_details):
        ...
        keybindings_content = '[{"key": "ctrl+shift+p", "command": "workbench.action.showCommands"}]'
    ...
    def test_pull_snippets_success(self, pull_command, temp_dirs, app_details):
        ...
        snippet_content = '{"test": {"prefix": "test", "body": "console.log(\\"test\\");"}}'
    ...
    def test_pull_settings_overwrite_behavior(self, mock_confirm, pull_command, temp_dirs, app_details, overwrite):
    ...
    @pytest.mark.parametrize("layer_type,layer_name,expected_path", [
        ("base", None, "base"),
    ...
    def test_resolve_target_layer_paths(self, pull_command, temp_dirs, layer_type, layer_name, expected_path):
        """Test various layer path resolutions."""
        if layer_type == "stack" and layer_name is None:
            with pytest.raises(VscSyncError):
                pull_command._resolve_target_layer_path(layer_type, layer_name, "test-app")
        else:
            target_path = pull_command._resolve_target_layer_path(layer_type, layer_name, "test-app")
    ...
    def test_validate_project_success(self, pull_command, temp_dirs):
        ...
        assert project_details.executable_path is None  # Projects don't have executables
    ...
    def test_validate_project_no_vscode_dir(self, pull_command, temp_dirs):
        ...
        with pytest.raises(VscSyncError, match="Project does not have a .vscode directory"):
    ...
    def test_run_requires_app_alias_or_project_path(self, pull_command):
        ...
        with pytest.raises(VscSyncError, match="app_alias is required when not pulling from a project"):
            pull_command.run(
                app_alias=None,
                layer_type="base",
                layer_name=None,
    ...
    def test_full_preview_mode(self, pull_command, temp_dirs):
        ...
        with patch.object(pull_command, '_show_content_with_pager') as mock_pager:
            ...
            assert kwargs.get('use_pager') is False  # no_pager=True means use_pager=False
    ...
    def test_show_content_with_pager_no_pager(self, pull_command):
        ...
        with patch('vsc_sync.commands.pull_cmd.console') as mock_console:
            pull_command._show_content_with_pager(test_content, "Test Content", use_pager=False)
    ...
    def test_show_content_with_pager_with_pager(self, mock_tempfile, mock_subprocess, pull_command):
        ...
        with patch('vsc_sync.commands.pull_cmd.console') as mock_console, \
             patch('vsc_sync.commands.pull_cmd.os.unlink'):
            
            pull_command._show_content_with_pager(test_content, "Test Content", use_pager=True)
